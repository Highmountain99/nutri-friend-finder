-- Reactivate general (email-less) invites that were consumed by the first patient
UPDATE public.patient_invitations
SET status = 'pending', accepted_by = NULL, accepted_at = NULL
WHERE patient_email IS NULL AND status = 'accepted';
...
begin
  ...
  -- General links (no email) stay usable indefinitely; personal invites must be pending
  select *
  into _invite
  ...
  -- Only single-use (email) invites are consumed
